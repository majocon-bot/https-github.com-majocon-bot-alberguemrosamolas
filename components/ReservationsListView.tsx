...
const ReservationsListView: React.FC<ReservationsListViewProps> = ({ 
  ...
  type DisplayMode = 'group' | 'individual' | 'active_all' | 'past_all';
  ...
  const allGroupedReservations = useMemo((): GroupedReservationWithCost[] => {
  ...
  const activeGrouped = useMemo(() => {
    return allGroupedReservations
      .filter(group => group.maxCheckOut >= todayStr)
      .sort((a, b) => a.minCheckIn.localeCompare(b.minCheckIn));
  }, [allGroupedReservations, todayStr]);

  const pastGrouped = useMemo(() => {
    return allGroupedReservations
      .filter(group => group.maxCheckOut < todayStr)
      .sort((a, b) => b.maxCheckOut.localeCompare(a.maxCheckOut));
  }, [allGroupedReservations, todayStr]);
  
  const activeIndividual = useMemo(() => {
    return individualReservations
      .filter(res => res.contractDetails.checkOutDate >= todayStr)
      .sort((a, b) => a.contractDetails.checkInDate.localeCompare(b.contractDetails.checkInDate));
  }, [individualReservations, todayStr]);

  const pastIndividual = useMemo(() => {
    return individualReservations
      .filter(res => res.contractDetails.checkOutDate < todayStr)
      .sort((a, b) => b.contractDetails.checkOutDate.localeCompare(a.contractDetails.checkOutDate));
  }, [individualReservations, todayStr]);
  
  const GroupReservationsList = ({ groups, emptyMessage }: { groups: GroupedReservationWithCost[], emptyMessage: string }) => (
    <>
      {groups.length > 0 ? (
  ...
  const IndividualReservationsList = ({ reservationsList, emptyMessage }: { reservationsList: IndividualReservation[], emptyMessage: string }) => {
    return (
  ...
  const buttonClass = (mode: DisplayMode) => 
    `w-48 px-4 py-2 rounded-md font-semibold text-sm transition-all ${displayMode === mode ? 'bg-white text-indigo-700 shadow' : 'bg-transparent text-slate-600'}`;
  ...
  return (
    <div className="space-y-8">
      <header className="text-center">
        ...
        <p className="text-xl text-slate-500 mt-2">Gestiona todas las reservas de la instalación.</p>
      </header>
      
      <div className="flex flex-wrap justify-center items-center gap-2 bg-slate-200 p-1 rounded-lg no-print">
        <button onClick={() => setDisplayMode('group')} className={buttonClass('group')}>Grupales (Activas)</button>
        <button onClick={() => setDisplayMode('individual')} className={buttonClass('individual')}>Individuales (Activas)</button>
        <button onClick={() => setDisplayMode('active_all')} className={buttonClass('active_all')}>Todas (Activas)</button>
        <button onClick={() => setDisplayMode('past_all')} className={buttonClass('past_all')}>Todas (Pasadas)</button>
      </div>
        ...
        {displayMode === 'group' && (
            <div className="space-y-4">
                <h2 className="text-3xl font-bold text-slate-800 text-center">
                    Reservas Grupales (Activas y Próximas)
                </h2>
                <GroupReservationsList groups={activeGrouped} emptyMessage="No hay reservas grupales activas o próximas para mostrar." />
            </div>
        )}
        {displayMode === 'individual' && (
            <div className="space-y-4">
                <h2 className="text-3xl font-bold text-slate-800 text-center">
                    Reservas Individuales (Activas y Próximas)
                </h2>
                <IndividualReservationsList reservationsList={activeIndividual} emptyMessage="No hay fichas de registro individuales activas o próximas para mostrar." />
            </div>
        )}
        {displayMode === 'active_all' && (
            <div className="space-y-8">
                <div className="space-y-4">
                    <h2 className="text-3xl font-bold text-slate-800 text-center">Reservas Grupales (Activas y Próximas)</h2>
                    <GroupReservationsList groups={activeGrouped} emptyMessage="No hay reservas grupales activas o próximas para mostrar." />
                </div>
                <div className="space-y-4">
                    <h2 className="text-3xl font-bold text-slate-800 text-center">Reservas Individuales (Activas y Próximas)</h2>
                    <IndividualReservationsList reservationsList={activeIndividual} emptyMessage="No hay fichas de registro individuales activas o próximas para mostrar." />
                </div>
            </div>
        )}
         {displayMode === 'past_all' && (
            <div className="space-y-8">
                <div className="space-y-4">
                    <h2 className="text-3xl font-bold text-slate-800 text-center">Reservas Grupales (Pasadas)</h2>
                    <GroupReservationsList groups={pastGrouped} emptyMessage="No hay reservas grupales pasadas para mostrar." />
                </div>
                <div className="space-y-4">
                    <h2 className="text-3xl font-bold text-slate-800 text-center">Reservas Individuales (Pasadas)</h2>
                    <IndividualReservationsList reservationsList={pastIndividual} emptyMessage="No hay fichas de registro individuales pasadas para mostrar." />
                </div>
            </div>
        )}
      </div>
